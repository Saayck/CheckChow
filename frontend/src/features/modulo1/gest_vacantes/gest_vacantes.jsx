...
import { contarVacantesEfectivas } from "../../../utils/admissionRanking";
...
export default function GestVacantes() {
	...
	const resultadosPorVacante = useMemo(() => {
		const map = new Map();
		resultados.forEach((r) => {
			const procesoId = r.proceso?.id;
			const carreraId = r.inscripcion?.carrera?.id;
			if (!procesoId || !carreraId) return;
			const key = `${procesoId}:${carreraId}`;
	...
	const vacanteStats = (v) => {
		const grupo = resultadosPorVacante.get(`${v.proceso?.id}:${v.carrera?.id}`) || [];
		if (!grupo.length) return { evaluados: 0, ingresos: 0, ampliadas: 0, efectivas: v.vacantes || 0 };
		const stats = contarVacantesEfectivas(grupo, v.vacantes, v.permiteAmpliacion, "puntajeFinal");
		return { evaluados: grupo.length, ...stats };
	};

	const totalVacantes = vacantesFiltradas.reduce((sum, v) => sum + (v.vacantes || 0), 0);
	const totalVacantesEfectivas = vacantesFiltradas.reduce((sum, v) => sum + vacanteStats(v).efectivas, 0);
					...
					<div className="row g-3 mb-4">
						...
						<div className="col-sm-4">
							<div className="glass-card p-4">
								...
								<h2 className="metric-value mb-0" style={{ color: "#22c55e" }}>
									{totalVacantesEfectivas > totalVacantes ? `${totalVacantes} + ${totalVacantesEfectivas - totalVacantes}` : totalVacantes}
								</h2>
							</div>
						</div>
						<div className="col-sm-4">
							...
							<table className="table table-dark table-borderless align-middle dashboard-table mb-0">
								<thead>
									<tr>
										...
										<th style={{ textAlign: "center" }}>Efectivas</th>
								...
								<tbody>
									...
									{!loading && vacantesFiltradas.map(v => {
										...
										<tr key={v.id} style={{ borderBottom: "1px solid rgba(148,163,184,0.07)" }}>
											...
											<td style={{ textAlign: "center", fontFamily: "monospace", color: "#94a3b8" }}>
												{stats.evaluados}
											</td>
											<td style={{ textAlign: "center", fontFamily: "monospace", color: stats.ampliadas ? "#eab308" : "#cbd5e1", fontWeight: 700 }}>
												{stats.efectivas}
												{stats.ampliadas > 0 && (
													<span style={{ display: "block", fontSize: "0.68rem" }}>+{stats.ampliadas} empate</span>
												)}
											</td>
